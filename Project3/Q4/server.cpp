...
void processCommand(ClientState &client, std::string buffer) {
    // remove trailing newline / carriage return
    while (!buffer.empty() && (buffer.back() == '\n' || buffer.back() == '\r')) buffer.pop_back();
    ...
    if (client.expectedPoints > 0) {
        // Reading points for Newgraph
        Point p;
        if (parsePoint(buffer, p)) {
            if (std::find(graph.begin(), graph.end(), p) == graph.end()) {
                graph.push_back(p);
                reply << "Point " << p.x << "," << p.y << " added.\n";
                client.expectedPoints--;
            } else {
        ...
        if (client.expectedPoints > 0)
            reply << "Remaining points: " << client.expectedPoints << "\n";
        send(client.sock, reply.str().c_str(), reply.str().size(), 0);
        return;
    }
    ...
    if (cmd == "EXIT" || cmd == "exit") {
        ...
        send(client.sock, reply.str().c_str(), reply.str().size(), 0);
        close(client.sock);
        client.sock = -1; // mark for removal
    ...
    if (cmd == "Newgraph") {
        int n; iss >> n;
        if (n <= 0) reply << "Invalid number of points.\n";
        else {
            graph.clear();
            client.expectedPoints = n;
        ...
        if (parsePoint(rest, p)) {
            if (std::find(graph.begin(), graph.end(), p) == graph.end()) {
                graph.push_back(p);
                reply << "Point " << p.x << "," << p.y << " added.\n";
            } else reply << "Point already exists.\n";
        } else reply << "Invalid format. Use x,y\n";
        ...
        if (parsePoint(rest, p)) {
            ...
            } else reply << "Point not found.\n";
        } else reply << "Invalid format. Use x,y\n";
    } else if (cmd == "CH") {
        auto hull = computeConvexHull(graph);
        double area = computePolygonArea(hull);
        reply << "area of the convex hull " << std::fixed << std::setprecision(6) << area << "\n";
    } else {
...
void sigintHandler(int) { serverRunning = false; }
    ...
    while (serverRunning) {
        ...
        if (FD_ISSET(STDIN_FILENO, &readfds)) {
            ...
            if (!std::getline(std::cin, line)) { // Ctrl+D
                std::cout << "EOF detected, shutting down server...\n";
                break;
            }
        ...
        for (auto &c : clients) {
            if (c.sock >= 0 && FD_ISSET(c.sock, &readfds)) {
                ...
                ssize_t bytesRead = recv(c.sock, buffer, BUFFER_SIZE - 1, 0);
                if (bytesRead <= 0) {
                    std::cout << "Client disconnected.\n";
                    close(c.sock);
                    c.sock = -1;
                } else {
                    processCommand(c, std::string(buffer));
                }
            }
        }

        clients.erase(std::remove_if(clients.begin(), clients.end(),
                     [](ClientState &c){ return c.sock == -1; }),
                     clients.end());
    }
